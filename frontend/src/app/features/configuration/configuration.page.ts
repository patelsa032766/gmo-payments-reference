...
export class ConfigurationPage implements OnInit {
  ...
  protected publish():void {
    if(!this.operatorToken){this.message.set('Enter the operator token to save changes.');return;}
    this.saving.set(true); this.api.saveConfigurationDraft(this.methods(),this.operatorToken).subscribe({next:draft=>{this.draftVersion.set(draft.version);this.api.publishConfiguration(this.operatorToken).subscribe({next:active=>{this.configuration.set(active);this.methods.set(structuredClone(active.methods));this.draftVersion.set(null);this.dirty.set(false);this.saving.set(false);this.message.set(`Saved. Version ${active.version} is now serving Checkout and will survive navigation and restart.`);},error:()=>this.failure('The draft was saved but could not be activated.')});},error:()=>this.failure('Changes could not be saved. Check the operator token and values.')});
  }
