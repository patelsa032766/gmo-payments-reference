...
export class MitPage implements OnInit {
  ...
  protected choose(id:string):void{this.selectedId.set(id);const item=this.selected();if(item){const customer=this.instruments().filter(i=>i.customerCode===item.customerCode);this.primaryId=customer.find(i=>i.preferenceRole==='PRIMARY')?.instrumentId??id;this.backupId=customer.find(i=>i.preferenceRole==='BACKUP')?.instrumentId??'';}}
  ...
  protected submit():void{const item=this.selected();if(!item||!this.operatorToken){this.message.set('Select an instrument and enter the operator token.');return;}if(!window.confirm(`Submit JPY ${this.amount.toLocaleString()} using ${item.maskedDisplay}?`))return;this.submitting.set(true);this.message.set(null);this.api.submitMit(item.instrumentId,this.amount,this.reference,this.execution,this.operatorToken,crypto.randomUUID()).subscribe({next:result=>{this.result.set(result);this.submitting.set(false);this.message.set(`${result.state} · ${result.transactionId}`);},error:()=>{this.submitting.set(false);this.message.set('The recurring payment could not be submitted. Check the operator token and instrument readiness.');}});}
  protected savePreferences():void{const item=this.selected();if(!item||!this.operatorToken)return;this.api.setPreferences(item.customerCode,this.primaryId,this.backupId||null,this.operatorToken).subscribe({next:()=>{this.message.set('Primary and backup preferences saved.');this.reload();},error:()=>this.message.set('Preferences could not be saved.')});}
