...
export class MitPage implements OnInit {
  ...
  protected submit():void{const item=this.selected();if(!item||!this.operatorToken){this.message.set('Select an instrument and enter the operator token.');return;}const mode=this.supportsFlexibleExecution(item)?this.execution:'CAPTURE';const label=mode==='AUTH'?'authorize now and capture later':'run an immediate sale';if(!window.confirm(`Submit JPY ${this.amount.toLocaleString()} using ${item.maskedDisplay} and ${label}?`))return;this.submitting.set(true);this.message.set(null);this.api.submitMit(item.instrumentId,this.amount,this.reference,mode,this.operatorToken,crypto.randomUUID()).subscribe({next:result=>{this.result.set(result);this.submitting.set(false);this.message.set(`${result.state} · ${result.transactionId}`);},error:()=>{this.submitting.set(false);this.message.set('The recurring payment could not be submitted. Check the operator token and instrument readiness.');}});}
