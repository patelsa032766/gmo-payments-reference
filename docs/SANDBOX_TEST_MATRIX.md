...
| Real-time bank debit (`口座直結決済`) | **Completed.** Registration returned through the public callback, followed by account inquiry, amount-bearing Entry, and Exec with `CAPTURE`. Example `TXN-BANKDIRECT-C8116221F68B`, JPY 20,000. | Not applicable. | The checkout collects bank and account-holder Kana; GMO's hosted page securely collects branch, account type, and account number. | This remains distinct from Koza Furikae in codes, UI, storage, endpoints, and lifecycle. |
| Koza Furikae Select (`口座振替（セレクト）`) | Not applicable. The first premium is Furikomi, not an immediate Koza debit. | Not applicable. | **Completed through hosted registration and first-payment instructions.** The browser return was verified by mandate inquiry, after which Java created a no-charge mandate thread and a separate amount-bearing Furikomi thread. Example mandate `TXN-KOZAFURIKAE-3745D4AA2E47`, linked first premium `TXN-FURIKOMI-LEGACY-00000043`. | Monthly-only. The browser return is only a locator; Java verifies it server-to-server before provisioning the mandate. The first payment remains unpaid/partially paid until Furikomi evidence arrives. |
...
| Koza future debit (single API or monthly batch) | Asynchronous scheduled debit only. | Not applicable. | Both submission modes use the same Entry/Exec application service and retain both exchanges. `TXN-KOZA-5F2DFA300BB2` proved both calls reached GMO: Entry succeeded and Exec rejected oversized `Remarks` with `M01331005`. Java now sends a GMO-safe 15-character description, keeps the full reference in `ClientField1`, and excludes the fixture mandate from live selection. `TXN-KOZA-311D0B335672` progressed from `SCHEDULED` to GMO `PAYSUCCESS`; because its earlier notification was not delivered, the operator's real `SearchTradeMulti` inquiry reconciled the local transaction and batch item to `PAID` while preserving inquiry provenance. | The single API is useful for ad hoc/operator testing; the batch groups the monthly run. Neither provider acceptance nor an HTTP 200 means money was collected. |
...
13. Real-time Bank Direct no longer asks for branch, account type, or account
