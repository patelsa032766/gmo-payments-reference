...
public final class MitPaymentService {
    ...
    public PaymentSubmissionResult submit(String instrumentId,long amountJpy,String reference,String key,Map<String,Object> details){
        ...
        String fingerprint=sha(instrumentId+"|"+amountJpy+"|"+reference+"|"+details);
        var reservation=repository.reserve(instrumentId,amountJpy,reference,key,fingerprint);
        if(reservation.replayed())return repository.findSubmission(reservation.context().transactionId()).orElseThrow();
        try{return repository.recordSuccess(reservation.context(),gateway.executeMit(reservation.context(),reservation.instrumentFacts(),details));}
