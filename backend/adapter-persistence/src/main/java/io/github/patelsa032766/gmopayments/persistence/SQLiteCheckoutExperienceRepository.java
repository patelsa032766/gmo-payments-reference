...
/** SQLite adapter for local test-customer selection and configuration-page authentication policy. */
@Repository
public class SQLiteCheckoutExperienceRepository implements CheckoutExperienceRepository {
    ...
    @Override public CheckoutExperienceSettings get() {
        var settings=jdbc.sql("""
                SELECT selected_application_number,configuration_token_required,checkout_language
    ...
    @Override public CheckoutExperienceSettings update(String applicationNumber,long amountJpy,
        ...
        lockRetry.execute("update checkout experience",()->transactions.execute(status->{
            ...
            jdbc.sql("""
                    ...
                    configuration_token_required=:required,checkout_language=:language,updated_at=:now WHERE id=1
                    """).param("application",applicationNumber).param("required",configurationTokenRequired)
