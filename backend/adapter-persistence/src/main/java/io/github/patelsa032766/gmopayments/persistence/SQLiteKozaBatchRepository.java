...
public class SQLiteKozaBatchRepository implements KozaBatchRepository {
    ...
    private Instrument instrument(String id){return jdbc.sql("""
        SELECT i.id,i.provider_member_reference,i.provider_instrument_reference,i.masked_display,i.metadata_json,c.id customer_id,c.customer_code,c.full_name,r.version,
               (SELECT t.transaction_id FROM payment_transaction t
                WHERE t.instrument_id=i.id AND t.transaction_role='MANDATE_REGISTRATION'
                ORDER BY t.id DESC LIMIT 1) root_transaction_id
        ...
        """).param("id",id).query((rs,n)->new Instrument(rs.getLong(1),rs.getString(2),rs.getString(3),rs.getString(4),parse(rs.getString(5)),rs.getLong(6),rs.getString(7),rs.getString(8),rs.getInt(9),rs.getString(10))).optional().orElseThrow(()->new IllegalArgumentException("Active Koza mandate not found: "+id));}
    ...
    private record Instrument(long id,String member,String providerReference,String masked,Map<String,Object> metadata,long customerId,String customerCode,String customerName,int version,String rootTransactionId){}
}
